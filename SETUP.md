...
6. **Authorized redirect URIs**:
   - Add: `http://localhost:3001/auth/callback` (Google OAuth login - local dev)
   - Add: `http://matterflow.local/api/auth/google/callback` (Drive integration)
   - Add: `https://yourdomain.com/auth/callback` (Google OAuth login - production)
   - Add: `https://yourdomain.com/api/auth/google/callback` (Drive integration - production)

   **Note:** Google OAuth requires `localhost` for login callbacks (not `.local` domains). The callback goes to the Next.js app (port 3001) which proxies to GoTrue internally.
7. Click "Create"
8. **⚠️ SAVE THESE**:
